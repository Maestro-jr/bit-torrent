#! /usr/bin/env python3

"""
Distributed File System
Server 1
Fonyuy Berka
Jan 2026
(FIXED: Proper LIST command handling)
"""

# ========================
# MODULES
# ========================
import os
import sys
import time
import socket
import pickle

# ========================
# STORAGE ROOT (SERVER 1)
# ========================
STORAGE_ROOT = r"C:\Users\HP\Desktop\Servers\Server 1"
CONF_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'dfs.conf')

# ========================
# ARGUMENT CHECK
# ========================
def check_args():
    if len(sys.argv) != 2:
        print("ERROR: Must supply port number")
        print("USAGE: python dfs1.py <port>")
        sys.exit(1)

# ========================
# CREATE USER DIRECTORY
# ========================
def new_dir(username):
    user_dir = os.path.join(STORAGE_ROOT, username)
    os.makedirs(user_dir, exist_ok=True)
    return user_dir

# ========================
# PUT FILE
# ========================
def put(conn, data, username):
    file_folder = data[1]
    chunk1_name = data[2]
    chunk2_name = data[3]

    user_dir = new_dir(username)
    file_dir = os.path.join(user_dir, file_folder)
    os.makedirs(file_dir, exist_ok=True)

    # Receive chunk 1
    chunk1 = conn.recv(1024 * 1024)
    with open(os.path.join(file_dir, chunk1_name), 'wb') as f:
        f.write(chunk1)

    # Receive chunk 2
    chunk2 = conn.recv(1024 * 1024)
    with open(os.path.join(file_dir, chunk2_name), 'wb') as f:
        f.write(chunk2)

    print("PUT complete")

# ========================
# LIST FILES (FIXED)
# ========================
def list_files(conn, username):
    """
    Returns logical filenames (not chunks)
    Example:
        myfile.txt
        video.mp4
    """
    user_dir = os.path.join(STORAGE_ROOT, username)

    if not os.path.isdir(user_dir):
        conn.send(pickle.dumps([]))
        return

    logical_files = set()

    for root, dirs, files in os.walk(user_dir):
        for file in files:
            # Remove chunk suffix (.1, .2, .3, .4)
            base = file.rsplit('.', 1)[0]
            logical_files.add(base)

    conn.send(pickle.dumps(sorted(logical_files)))

# ========================
# GET FILE
# ========================
def get(conn, data, username):
    filename = data[1]
    user_dir = os.path.join(STORAGE_ROOT, username)

    found_chunks = []

    for root, dirs, files in os.walk(user_dir):
        for file in files:
            if file.startswith(filename + "."):
                found_chunks.append(os.path.join(root, file))

    if not found_chunks:
        conn.send(pickle.dumps([]))
        return

    conn.send(pickle.dumps(found_chunks))

    for chunk_path in found_chunks:
        with open(chunk_path, 'rb') as f:
            conn.sendall(f.read())
            time.sleep(0.05)

# ========================
# AUTHENTICATION
# ========================
def authenticate(conn):
    auth = conn.recv(1024).decode().strip()
    try:
        username, password = auth.split()
    except ValueError:
        conn.send(b"ERROR")
        return None

    with open(CONF_FILE, "r") as f:
        for line in f:
            user, pwd = line.split()
            if user == username and pwd == password:
                conn.send(b"OK")
                return username

    conn.send(b"ERROR")
    return None

# ========================
# MAIN SERVER LOOP
# ========================
def main():
    check_args()
    port = int(sys.argv[1])

    server = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    server.bind(("0.0.0.0", port))
    server.listen(5)

    print(f"DFS1 running on port {port}")
    print(f"Storage root: {STORAGE_ROOT}")

    while True:
        conn, addr = server.accept()
        print("Client connected from", addr)

        username = authenticate(conn)
        if not username:
            conn.close()
            continue

        data = conn.recv(1024).decode().split()
        if not data:
            conn.close()
            continue

        command = data[0].upper()

        if command == "PUT":
            put(conn, data, username)
        elif command == "LIST":
            list_files(conn, username)
        elif command == "GET":
            get(conn, data, username)
        else:
            conn.send(b"ERROR")

        conn.close()

# ========================
# ENTRY POINT
# ========================
if __name__ == "__main__":
    main()
